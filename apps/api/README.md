...
- Render deploys should validate DB config before build and before `npm run release:migrate`, then keep runtime start on `npm run start:server` (`npm run start` locally).
