            ...
            <div className="flex h-full min-h-0 flex-col">
              <div className="px-5 py-5 sm:px-6">
                <div className="mb-5 flex items-start justify-between gap-4">
                  <div>
                    <p className="text-label mb-3">Day Breakdown</p>
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  ...
                                  <p className="mt-2 text-sm text-muted-foreground">
                                    {trade.setup || "No setup"} {trade.session ? `• ${trade.session}` : ""}
                                  </p>
                                </div>
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div className="rounded-[1.35rem] border border-border/60 bg-background/60 px-4 py-4 dark:bg-white/[0.03]">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {sessions.length > 0 ? sessions.map((session) => <DataBadge key={session}>{session}</DataBadge>) : <span className="text-sm text-muted-foreground">No sessions tagged</span>}
                          </div>
                        </div>
                        <div className="rounded-[1.35rem] border border-border/60 bg-background/60 px-4 py-4 dark:bg-white/[0.03]">
                          <p className="text-label mb-3">Emotions</p>
                          <div className="flex flex-wrap gap-2">
                            {emotions.length > 0 ? emotions.map((emotion) => <DataBadge key={emotion}>{emotion}</DataBadge>) : <span className="text-sm text-muted-foreground">No emotions tagged</span>}
                          </div>
                        </div>
                      </div>
                    </section>
